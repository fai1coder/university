...
__kernel void heatpchm3d_dynamic_kernel(__global float *in, __global float *out,  __global float *f, __global float *i, __private int Nr, __private float h, __private float a,__private float t,__private float C,__private float p,__private float k0,__private float k1, __private float b, __private float lambda, __private int row_num, __private int total_num){
	...
	if (id+Nr<total_num){
		...
		if (excess!= 0 && excess != Nr-1 && id>=Nr) {
			...
			out[id] = in[id] + mad(a*t, ((in[id + 1] + in[id - 1] + in[id + Nr] + in[id - Nr] - 4 * in[id]) / (h*h) + (1 / r) * (in[id + 1] - in[id - 1]) / (2 * h)), Q*t / (C*p));
		}
	}
	f[id] = -b*t*i[id] * f[id] + f[id];

	barrier(CLK_GLOBAL_MEM_FENCE | CLK_LOCAL_MEM_FENCE);
	if (id < Nr) out[id] = lambda*out[id + Nr] / (lambda - 10.0*h);
	if (id >= Nr*(total_num/Nr - 1)) out[id] = lambda*out[id - Nr] / (lambda- 10.0*h);
